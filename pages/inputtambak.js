...
export default class InputTambak extends React.Component {
  state = {
    tambak : {
      id: 1,
      nama_tambak: "",
  ...
  handlePost = () => {
    Fire.shared
    .addPost({ 
      id:this.state.tambak.id,
      nama_tambak: this.state.nama_tambak.trim(), 
      banyak_kincir: this.state.banyak_kincir.trim(),
      ketinggian: this.state.ketinggian.trim(),
      luas: this.state.luas.trim(),
      ph_air: this.state.ph_air.trim(),
      tabur_benih: this.state.tabur_benih.trim(),
      date: this.state.date,
    })
    .then(ref => {
      this.setState({ nama_tambak: "", banyak_kincir: "", ketinggian:"", luas: "", ph_air: "", tabur_benih: "", date: null});
  ...
  componentDidMount () {
    const currentTambak = this.props.navigation.getParam('tambak');
    console.log(currentTambak);

    if(currentTambak) {
      this.setState(prevState => ({tambak: prevState.tambak = currentTambak}))
    }
  }
    ...
    return (
      <View style={styles.container}>
        ...
        <View style={styles.content}>
          {/* Judul Page */}
          <Text style={styles.tittlecontent}>Input Tambak</Text>
            {/* Form Nama Tambak */}
